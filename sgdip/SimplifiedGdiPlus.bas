...
Public Declare Function GdipCreateBitmapFromHBITMAP Lib "gdiplus" Alias "GdipCreateBitmapFromHBitmap" (ByVal hBm As Long, ByVal hPal As Long, Bitmap As Long) As GpStatus
Public Declare Function GdipCreateHBITMAPFromBitmap Lib "gdiplus" Alias "GdipCreateHBitmapFromBitmap" (ByVal Bitmap As Long, hBmReturn As Long, ByVal Background As Long) As GpStatus
Public Declare Function GdipCreateBitmapFromHICON Lib "gdiplus" Alias "GdipCreateBitmapFromHIcon" (ByVal hIcon As Long, Bitmap As Long) As GpStatus
Public Declare Function GdipCreateHICONFromBitmap Lib "gdiplus" Alias "GdipCreateHIconFromBitmap" (ByVal Bitmap As Long, hBmReturn As Long) As GpStatus
...
Private Declare Function CLSIDFromString Lib "ole32.dll" Alias "ClsIDFromString" (ByVal lpszProgID As Long, pCLSID As ClsID) As Long
...
'新建贴图画刷
Public Function NewChartletBrush(ByVal hImage As Long, ByRef DatumPoint As PointL, Optional ByVal mWrapMode As WrapMode = WrapModeTile, Optional ByVal ReleaseHandles As Boolean = True)
    InitGDIPlus
    Dim Retn As Long
...
'根据路径创建对应的区域
Public Function NewRegionFromPath(ByVal hPath As Long, ByVal mCombineMode As CombineMode, Optional ByVal ReleaseHandles As Boolean = True) As Long
    Dim Retn As Long
    InitGDIPlus
    Retn = NewRegion
...
'保存图像文件
Public Sub SaveImageFile(ByVal hImage As Long, ByVal FilePath As String, Optional ByVal FileEncoder As EncoderValue = EncoderValueColorTypeRGB, Optional ByVal ReleaseHandles As Boolean = True)
    InitGDIPlus
...
'图像拷贝（根据hDC，可以实现双缓冲）
Public Sub CopyGraphics(ByRef hDCSource As Long, ByRef hDcDestination As Long, ByRef RectDestination As RectL, ByRef DatumPoint As PointL)
    BitBlt hDcDestination, RectDestination.Left, RectDestination.Top, RectDestination.Right, RectDestination.Bottom, hDCSource, DatumPoint.X, DatumPoint.Y, vbSrcCopy
End Sub

'在画布上绘制简单文本
Public Sub DrawSimpleText(ByVal hGraphics As Long, ByVal Text As String, ByRef mFont As FontType, ByRef DatumPoint As PointL, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
...
'在画布上绘制遮罩文本(以百分比计算)
Public Sub DrawMaskedText(ByVal hGraphics As Long, ByVal Text As String, ByRef mFont As FontType, ByRef DatumPoint As PointL, ByVal Percentage As Single, ByVal hBorder As Long, ByVal hFill As Long, ByVal hMask As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
    ...
    Dim tCodePointsFitted As Long, tLinesFilled As Long                         '临时需要的变量
    If Text = "" Then Exit Sub
    InitGDIPlus
    GdipSetSmoothingMode hGraphics, SmoothingModeAntiAlias                      '抗锯齿处理
    GdipCreateStringFormat 0, 0, hStringFormat                                  '创建字符串格式
    ...
    mRect.Right = mRect.Right * Percentage / 100                                '计算遮罩百分比
...
'在画布上绘制直线
Public Sub DrawLine(ByVal hGraphics As Long, ByRef Point1 As PointL, ByRef Point2 As PointL, ByVal hPen As Long, Optional ByVal ReleaseHandles As Boolean = True)
    InitGDIPlus
    GdipSetSmoothingMode hGraphics, SmoothingModeAntiAlias                      '抗锯齿处理
...
'在画布上绘制矩形
Public Sub DrawRectangle(ByVal hGraphics As Long, ByRef mRect As RectL, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
    Dim hPath  As Long
    InitGDIPlus
    GdipSetSmoothingMode hGraphics, SmoothingModeAntiAlias                      '抗锯齿处理
    hPath = NewPath
...
'在画布上绘制椭圆（包含圆形）
Public Sub DrawEllipse(ByVal hGraphics As Long, ByRef mRect As RectL, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
    Dim hPath As Long
    InitGDIPlus
    GdipSetSmoothingMode hGraphics, SmoothingModeAntiAlias                      '抗锯齿处理
    hPath = NewPath
...
'在画布上绘制圆角矩形
Public Sub DrawRoundedRectangle(ByVal hGraphics As Long, ByRef mRect As RectL, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal FilletRadius As Long = -1, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
...
'在画布上绘制多边形
Public Sub DrawPolygon(ByVal hGraphics As Long, ByRef mPoint() As PointL, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
...
'在画布上绘制图像
Public Sub DrawImage(ByVal hGraphics As Long, ByVal hImage As Long, ByRef DatumPoint As PointL, Optional ByVal TransformMode As RotateFlipType = RotateNoneFlipNone, Optional ByVal Zoom As Single = 1#, Optional ByVal ReleaseHandles As Boolean = True)
    Dim imgWidth As Long, imgHeight As Long
    If Zoom <= 0 Then
...
Public Sub DrawTile(ByVal hGraphics As Long, ByVal hImage As Long, ByRef DatumPoint As PointL, ByRef SourceRect As RectL, Optional ByVal Zoom As Single = 1#, Optional ByVal ReleaseHandles As Boolean = True)
...
'在画布上绘制指定的路径
Public Sub DrawPath(ByVal hGraphics As Long, ByVal hPath As Long, ByVal hBorder As Long, ByVal hFill As Long, Optional ByVal DrawBorder As Boolean = True, Optional ByVal ReleaseHandles As Boolean = True)
    InitGDIPlus
    GdipFillPath hGraphics, hFill, hPath                                        '填充
    If DrawBorder Then GdipDrawPath hGraphics, hBorder, hPath                   '描边
    If ReleaseHandles Then
...
'用指定颜色涂满整个画布
Public Sub FillWholeGraphics(ByVal hGraphics As Long, Optional ByVal mColor As Long = &HFFFFFFFF, Optional ByVal ReleaseHandles As Boolean = True)
    InitGDIPlus
...
'创建基本形状的路径，其中FilletRadius当且仅当mShape参数为ShapeTypeRoundedRectangle时生效
Public Function CreateBasicShapePath(ByRef mRect As RectL, ByVal mShape As ShapeType, Optional ByVal FilletRadius As Long = 0) As Long
    InitGDIPlus
    Dim hPath As Long, RoundSize As Long, tPath(7) As Long, T As Long
    Dim mLeft As Long, mTop As Long, mWidth As Long, mHeight As Long
    hPath = NewPath(FillMode.FillModeWinding)
    ...
    Case ShapeTypeRoundedRectangle
        ...
        Next T
    End Select
    CreateBasicShapePath = hPath
End Function

'判断点是否在区域上
Public Function IsPointOnRegion(ByVal hGraphics As Long, ByVal hRegion As Long, ByRef mPoint As PointL, Optional ByVal ReleaseHandles As Boolean = True) As Boolean
    InitGDIPlus
    Dim Retn As Long
